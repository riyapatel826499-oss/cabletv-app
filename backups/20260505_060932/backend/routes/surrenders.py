...
def surrender_customer(customer_id: str, req: SurrenderRequest = SurrenderRequest(), current_user=Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        stbs = conn.execute(f"SELECT stb_no FROM connections WHERE customer_id = ? AND {flt}", [customer_id]).fetchall()
        ...
        if current_user["role"] in ["admin", "support"]:
            ...
            for stb_no in stb_list:
                existing = conn.execute(f"SELECT id FROM stb_inventory WHERE stb_no = ? AND {flt}", [stb_no]).fetchone()
                if existing:
                    conn.execute(f"UPDATE stb_inventory SET status = 'available', notes = ? WHERE stb_no = ? AND {flt}",
                                [f"Returned from surrendered customer {customer_id}", stb_no])
                else:
                    conn.execute(f"""INSERT INTO stb_inventory (stb_no, status, added_at, notes, operator_id)
                                    VALUES (?, 'available', ?, ?, ?)""",
                                [stb_no, now, f"Returned from surrendered customer {customer_id}", _oid])
            conn.commit()
            return {
        ...
        conn.execute(f"""INSERT INTO surrender_requests 
                     ...
                     [customer_id, cust["name"], ",".join(stb_list), req.reason,
                      current_user["id"], current_user["name"], now, _oid])
        ...
        if req.action == "approve":
            ...
            for stb_no in stb_list:
                ...
                if existing:
                    ...
                    conn.execute(f"""INSERT INTO stb_inventory (stb_no, status, added_at, notes, operator_id)
                                    VALUES (?, 'available', ?, ?, ?)""",
                                [stb_no, now, f"Approved surrender - customer {customer_id}", _oid])
...
def reactivate_customer(customer_id: str, current_user=Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        for row in stbs:
            if row["stb_no"]:
                conn.execute(f"DELETE FROM stb_inventory WHERE stb_no = ? AND {flt}", [row["stb_no"]])
