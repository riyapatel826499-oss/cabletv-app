    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <p style={{ fontSize: '0.88rem', color: 'var(--text-light)', marginTop: 2 }}>
          {t('Manage connection statuses, temp disconnects, and reconnections')}
        </p>
      </div>

      {/* Stat Cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16 }}>
        <StatBox icon={Wifi} label={t('Active')} value={stats?.total_connections ?? '--'} color="#34c759" />
        <StatBox icon={WifiOff} label={t('Temp Disconnected')} value={tdCount} color="#ff9f0a" />
        <StatBox icon={Users} label={t('Total Customers')} value={stats?.total_customers ?? '--'} color="#0071e3" />
      </div>
      ...
      <div className="glass-card animate-fade-in" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '16px 24px', borderBottom: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          <h2 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {t('Temp Disconnected Customers ({n})', { n: tdCount })}
          </h2>
        ...
        {isLoading ? (
          ...
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-light)' }}>
            ...
            {search ? t('No matching customers') : t('No temp disconnected customers')}
          </div>
        ) : (
                ...
                {filtered.map(c => (
                  <tr key={c.customer_id}>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      ...
                      >
                        <RefreshCw style={{ width: 14, height: 14 }} /> {t('Reconnect')}
                      </button>
                    </td>
                  </tr>
                ))}
          ...
          <div onClick={e => e.stopPropagation()} className="glass-card animate-fade-in" style={{ padding: 28, borderRadius: 16, maxWidth: 400, width: '90%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 8 }}>
                <RefreshCw style={{ width: 20, height: 20, color: '#34c759' }} /> {t('Reconnect Customer')}
              </h3>
            ...
            <div style={{ marginBottom: 16, padding: 14, borderRadius: 10, background: 'var(--bg-secondary)' }}>
              ...
              <p style={{ fontSize: '0.78rem', color: 'var(--text-light)', marginTop: 2 }}>{reconnectCustomer.customer_id} &middot; {reconnectCustomer.phone || t('No phone')}</p>
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: '0.78rem', fontWeight: 500, color: 'var(--text-light)', marginBottom: 6, display: 'block' }}>{t('STB Number')}</label>
              <input
                type="text"
                value={reconnectStb}
                onChange={e => setReconnectStb(e.target.value)}
                placeholder={t('Enter STB number')}
                style={{
              ...
              {reconnectCustomer.reclaimed_stb && (
                <p style={{ fontSize: '0.72rem', color: 'var(--text-light)', marginTop: 4 }}>
                  {t('Original STB: {stb} (pre-filled)', { stb: reconnectCustomer.reclaimed_stb })}
                </p>
              )}
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              >
                {reconnectMut.isPending ? t('Reconnecting...') : t('Reconnect')}
              </button>
            </div>
            {reconnectMut.isError && (
              <p style={{ fontSize: '0.78rem', color: '#ff3b30', marginTop: 10, textAlign: 'center' }}>
                {String(reconnectMut.error?.message || t('Reconnect failed'))}
              </p>
            )}
          </div>
        </div>
