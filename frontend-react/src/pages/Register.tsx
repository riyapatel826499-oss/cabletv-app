...
export default function Register() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrMsg(detail || 'Something went wrong. Try again.');
      setStep('error');
    } finally {
      setBusy(false);
    }
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: '#f5f7fa' }}>
        <div style={{ ...card, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: 8 }}>Welcome aboard!</div>
          ...
          <div style={{ background: '#f0fdf4', borderRadius: 12, padding: 14, marginBottom: 20, border: '1px solid #bbf7d0' }}>
            <div style={{ fontSize: '.8rem', color: '#6b7280', marginBottom: 4 }}>Your login</div>
            <div style={{ fontWeight: 700, fontSize: '1rem' }}>{result.username}</div>
            <div style={{ fontSize: '.75rem', color: '#9ca3af', marginTop: 4 }}>(the password you set)</div>
          </div>
          <button onClick={() => navigate('/app/login')}
            ...
            Go to Login <ArrowRight size={18} style={{ verticalAlign: 'middle', marginLeft: 6 }} />
          </button>
        </div>
      </div>
    );
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: '#f5f7fa' }}>
      <div style={card}>
        <div style={{ textAlign: 'center', marginBottom: 22 }}>
          ...
          <div style={{ fontSize: '1.15rem', fontWeight: 800 }}>Start Your Cable TV Billing</div>
          <div style={{ color: '#6b7280', fontSize: '.85rem' }}>Create your account — it's free to try</div>
        </div>

        <form onSubmit={submit}>
          <div style={{ marginBottom: 14 }}>
            <label style={lbl}><Building2 size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} /> Business name</label>
            <input style={inp} value={form.business_name} onChange={e => update('business_name', e.target.value)}
              placeholder="SSN Cables" required />
          </div>
          <div style={{ marginBottom: 14 }}>
            <label style={lbl}><User size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} /> Owner name</label>
            <input style={inp} value={form.owner_name} onChange={e => update('owner_name', e.target.value)}
              placeholder="Your name" required />
          </div>
          <div style={{ marginBottom: 14 }}>
            <label style={lbl}><Phone size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} /> Mobile number</label>
            <input style={inp} value={form.phone} onChange={e => update('phone', e.target.value)}
              placeholder="9876543210" inputMode="numeric" required />
          </div>
          <div style={{ marginBottom: 14 }}>
            <label style={lbl}><AtSign size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} /> Email (optional)</label>
            <input style={inp} value={form.email} onChange={e => update('email', e.target.value)}
              placeholder="owner@example.com" type="email" />
          </div>
          <div style={{ marginBottom: 14 }}>
            <label style={lbl}><MapPin size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} /> Service area (optional)</label>
          ...
          <div style={{ marginBottom: 18, padding: 14, background: '#f0f5ff', borderRadius: 12, border: '1px solid #dbeafe' }}>
            <div style={{ fontWeight: 700, fontSize: '.85rem', marginBottom: 10, color: '#1e40af' }}>Admin login credentials</div>
            <div style={{ marginBottom: 10 }}>
              <label style={lbl}><User size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} /> Username</label>
            ...
            <div>
              <label style={lbl}><KeyRound size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} /> Password</label>
              <input style={inp} value={form.admin_password} onChange={e => update('admin_password', e.target.value)}
                placeholder="Min 4 characters" type="password" required />
            </div>
          </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 18, fontSize: '.8rem', color: '#6b7280' }}>
          Already have an account? <Link to="/app/login" style={{ color: '#5aa2ff', fontWeight: 700, textDecoration: 'none' }}>Log in</Link>
        </div>
      </div>
